...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Space to toggle play/pause (simulated by focusing iframe)
      if (e.code === 'Space') {
          e.preventDefault();
          if (showNextUp) handlePlayNext(); // Space confirms next up
  ...
  useEffect(() => {
    ...
    timeIntervalRef.current = setInterval(() => {
        setSimulatedTime(prev => {
            ...
            if (duration > 0 && nextTime >= duration && nextItem && !showNextUp) {
                setShowNextUp(true);
            }
            return nextTime;
        });
  ...
  }, [loading, error, duration, nextItem, showNextUp, completedTriggered, onComplete]);
  ...
  const secureUrl = useMemo(() => {
    try {
        ...
        const allowedDomains = ['youtube.com', 'www.youtube.com', 'youtu.be', 'drive.google.com'];
            ...
            />
             ...
             <div className="absolute inset-0 flex items-center justify-center z-20">
                 <div className="bg-[#161b22] border border-red-500/30 px-8 py-6 rounded-2xl flex flex-col items-center gap-4 shadow-2xl">
                     ...
                     <button onClick={onClose} className="mt-4 px-6 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold transition">
                         חזרה
                     </button>
                </div>
            </div>
                    ...
                    <div className="p-5 relative z-10 -mt-12">
                        ...
                        <div className="flex gap-3">
                            <button 
                                ...
                                autoFocus
                                className="flex-1 bg-white hover:bg-cyan-400 text-black font-bold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-all shadow-lg"
                            >
                            ...
                            <button 
                                ...
                                className="px-4 py-2.5 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold transition-all border border-white/5"
                            >
                                ביטול
                            </button>
                        </div>
            ...
            <div className="absolute inset-0 z-[60] bg-black/80 flex items-center justify-center backdrop-blur-sm animate-fade-in">
                <div className="bg-[#161b22] border border-white/10 p-8 rounded-2xl max-w-md text-center shadow-2xl">
                    ...
                    <button 
                        ...
                        className="bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 px-8 rounded-xl transition"
                    >
                        כן, המשך בצפייה
                    </button>
                </div>
             ...
             <div className="absolute inset-0 z-[70] bg-black/80 flex items-center justify-center backdrop-blur-sm animate-fade-in" onClick={() => setShowShortcuts(false)}>
                 <div className="bg-[#161b22] border border-white/10 p-8 rounded-2xl max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
                     ...
                     <div className="grid grid-cols-2 gap-x-12 gap-y-4 text-gray-300">
                         ...
                         <div className="flex justify-between"><span>סגירה</span> <kbd className="bg-white/10 px-2 rounded">Esc</kbd></div>
                     </div>
                     <button onClick={() => setShowShortcuts(false)} className="mt-8 w-full py-2 bg-white/5 hover:bg-white/10 rounded-lg text-white font-bold transition">סגור</button>
                 </div>
             </div>
        )}
            ...
            >
                <div className="max-w-5xl mx-auto flex items-center justify-between pointer-events-auto">
                    <div className="flex items-center gap-4">
                        <button className="text-white/80 hover:text-white transition p-2 hover:bg-white/10 rounded-full" onClick={toggleMute}>
                    ...
                    <div className="flex items-center gap-6">
                        <button className="text-white/70 hover:text-white transition hover:scale-110 flex flex-col items-center gap-1 group">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 relative">
                        ...
                        <div className="relative">
                            <button 
                                className={`text-white/80 hover:text-white transition p-2 hover:bg-white/10 rounded-full ${showSettings ? 'bg-white/20 text-white' : ''}`}
                            ...
                            {showSettings && (
                                <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 w-64 bg-[#161b22]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-fade-in origin-bottom">
                                    <div className="p-2 space-y-1">
                                        <div className="p-3 hover:bg-white/5 rounded-lg group cursor-pointer" onClick={() => {
                                        ...
                                        <div className="p-3 hover:bg-white/5 rounded-lg group cursor-pointer" onClick={() => setShowSubtitles(!showSubtitles)}>
                                            <div className="flex items-center justify-between text-sm text-gray-300 group-hover:text-white">
                                        ...
                                        <div className="p-3 hover:bg-white/5 rounded-lg group cursor-pointer" onClick={() => setAspectRatio(prev => prev === 'cover' ? 'contain' : 'cover')}>
                                            <div className="flex items-center justify-between text-sm text-gray-300 group-hover:text-white">
                        ...
                        <button className="text-white/80 hover:text-white transition p-2 hover:bg-white/10 rounded-full hidden md:block" onClick={togglePiP} title="תמונה בתוך תמונה">
                            <PictureInPicture className="w-6 h-6" />
                        </button>

                        <button className="text-white/80 hover:text-white transition p-2 hover:bg-white/10 rounded-full" onClick={toggleFullscreen}>
