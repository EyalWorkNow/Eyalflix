...
export const Navbar: React.FC<NavbarProps> = ({ activePage, onNavigate, onSurpriseMe, currentMood, onMoodChange }) => {
  ...
  return (
    ...
    >
        <div className={`px-4 md:px-8 flex items-center justify-between transition-all duration-500 ${isScrolled ? 'h-16' : 'h-16 md:h-24'}`}>
          ...
          <div className="flex items-center gap-4 md:gap-8">
              ...
              {/* Desktop Nav Links */}
              {user && (
                  <nav className={`hidden md:flex items-center gap-1 p-1 rounded-full transition-all duration-500 ${isScrolled ? '' : 'bg-white/5 border border-white/5 backdrop-blur-sm'}`}>
                      {navLinks.map(link => (
                          <button
                            key={link.id}
          ...
          {user && activePage === 'home' && onMoodChange && (
              <div className={`hidden xl:flex items-center gap-2 p-1.5 rounded-full transition-all duration-500 ${isScrolled ? 'bg-transparent' : 'bg-black/20 backdrop-blur-md border border-white/10 shadow-lg'} absolute left-1/2 -translate-x-1/2`}>
                  {moods.map((mood) => {
                      ...
                      return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-300 ${
                                isActive 
                                ? 'bg-cyan-500 text-black shadow-[0_0_15px_rgba(34,211,238,0.4)] scale-105' 
