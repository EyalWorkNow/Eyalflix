...
import { Mail, Lock, ArrowRight, AlertTriangle, ShieldCheck, Check } from 'lucide-react';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isRegistering) {
                ...
                if (success) {
                    setSuccessMsg('✨ נרשמת בהצלחה! מיד נפתח את תהליך ההתאמה האישית...');
                    setTimeout(() => {
                ...
                if (success) {
                    setSuccessMsg('🎉 התחברת בהצלחה! טוען את הפרופיל שלך...');
            ...
            if (!success) {
                setErrorMsg(isRegistering ? 'נכשל ביצירת חשבון. האימייל אולי כבר בשימוש.' : 'שם משתמש או סיסמה שגויים.');
                handleFailure();
            }
        } catch (error) {
            setErrorMsg('שגיאה בחיבור לשרת. נסה שוב.');
            handleFailure();
        } finally {
            setLoading(false);
        }
        ...
        <div className="min-h-screen relative flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md px-4 animate-fade-in">
                ...
                <div className="bg-[#161b22]/80 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl ring-1 ring-white/5">
                    ...
                    {/* Success Message Box */}
                    {successMsg && (
                        <div className="mb-6 bg-green-500/10 border border-green-500/20 rounded-xl p-4 flex items-start gap-3 animate-fade-in">
                            <Check className="w-5 h-5 text-green-500 shrink-0 mt-0.5" />
                            <p className="text-sm text-green-200 font-medium">{successMsg}</p>
                        </div>
                    )}
